// TRUSTBOT ALPHA — STATIC (Protected)
// Identifier (single source of truth):
const TRUSTBOT_IDENTIFIER = "DRR-TRUSTBOT-ALPHA-CEC1-SENTINEL-FCPS1.0-2025-09-06";
...
        {id:"mailingAddress",label:"Mailing Address",type:"text",placeholder:"350 Northern Blvd, Albany, NY 12204, Ste 324"},
        {id:"jurisdiction",label:"Jurisdiction Statement",type:"textarea",default:"This trust is organized and administered under common law. Any state references are administrative only."}
      ]
    }
  ]
};

// Simple templates (no server needed)
...
<p>This private trust is administered under common law. Any reference to state or federal systems is solely for administrative purposes and does not confer jurisdiction absent express consent.</p>
...
// stamp header info
idEl.textContent = TRUSTBOT_IDENTIFIER;
ownerEl.textContent = TRUSTBOT_OWNER;
disclaimerEl.textContent = SCHEMA.disclaimer;
...
    let input;
    if (f.type === "textarea"){
      input = `<textarea name="${f.id}" ${req} placeholder="${f.placeholder||""}">${f.default||""}</textarea>`;
    } else {
      input = `<input type="${f.type||"text"}" name="${f.id}" value="${f.default||""}" ${req} placeholder="${f.placeholder||""}" />`;
    }
